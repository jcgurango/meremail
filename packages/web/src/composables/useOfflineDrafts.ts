...
async function syncDraft(draft: OfflineDraft): Promise<void> {
  ...
  if (draft.serverId) {
    ...
    try {
      const response = await fetch(`/api/drafts/${draft.serverId}`)
      if (!response.ok) {
        if (response.status === 404) {
          // Draft was deleted on server - recreate
      ...
      const serverDraft = await response.json() as ServerDraft
      const merged = mergeDraft(draft, serverDraft)

      // Update server with merged data
      const patchResponse = await fetch(`/api/drafts/${draft.serverId}`, {
        method: 'PATCH',
      ...
      if (!patchResponse.ok) {
        throw new Error(`Failed to update draft: ${patchResponse.status}`)
      }
