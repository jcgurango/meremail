...
async function search(reset = true) {
  ...
  try {
    ...
    const data = await $fetch<{ results: EmailResult[], hasMore: boolean }>('/api/search', {
      query: params
    })
...
if (senderId.value) {
  $fetch<{ contact: { id: number; name: string | null; email: string } }>(`/api/contacts/${senderId.value}`)
    .then(data => {
