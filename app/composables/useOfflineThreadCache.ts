...
export async function cacheSetAsideThreads(): Promise<void> {
  ...
  try {
    ...
    const data = await $fetch<{ emails: SetAsideEmail[]; hasMore: boolean }>('/api/set-aside', {
      query: { limit: 100 }
    })
...
export async function cacheReplyLaterThreads(): Promise<void> {
  ...
  try {
    ...
    const data = await $fetch<{ threads: ServerThread[]; hasMore: boolean }>('/api/threads', {
      query: { replyLater: 'true', limit: 50 }
    })
    ...
    for (const thread of data.threads) {
      ...
      try {
        const threadDetail = await $fetch<ServerThreadDetail>(`/api/threads/${thread.id}`)
...
export async function cacheInboxThreads(): Promise<void> {
  ...
  try {
    // Fetch inbox threads
    const data = await $fetch<{ threads: ServerThread[]; hasMore: boolean }>('/api/threads', {
      query: { bucket: 'approved', limit: CACHE_LIMITS.inboxThreads }
    })
